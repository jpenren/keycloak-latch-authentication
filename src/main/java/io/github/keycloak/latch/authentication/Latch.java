...
public final class Latch {
    ...
    private static Logger log = Logger.getLogger(MigrationModelManager.class);
	 ...
	 */
    public PairResponse pair(String token) {
     ...
     * @throws MethodException
     */
    public StatusResponse status(String accountId) {
    ...
    private String doGet(String query) {
    	...
    	try {
			...
			throw new IllegalStateException("Latch API communication error: ", e);
		}finally{
			try {
				connection.close();
			} catch (Exception e2) {
     ...
     * @throws IOException
     */
    private CloseableHttpClient initializeConnection() {
    	try {
			...
			throw new IllegalStateException("Error initializing connection: ", e);
		}
    }
